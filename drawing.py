...
class Draw:
    ...
    def plot_regional_deposits(self, deposits: npt.NDArray, mean: float, name: str):
        im = plt.imshow(
            deposits.reshape(16, 16), vmin=0, vmax=deposits.max(), cmap="Purples"
        )
        ax = plt.gca()
        cbar = ax.figure.colorbar(im, ax=ax)
        cbar.ax.set_ylabel(r"Calorimeter E$_T$ deposit (GeV)")
        plt.xticks(np.arange(16), labels=np.arange(4, 16))
        plt.yticks(
            np.arange(16),
    ...
    def plot_reconstruction_results(
        ...
        deposits_in = deposits_in.reshape(16, 16)
        deposits_out = deposits_out.reshape(16, 16)
        ax1 = plt.subplot(1, 4, 1)
        ax1.get_xaxis().set_visible(False)
        ax1.get_yaxis().set_visible(False)
        ax1.set_title("Original", fontsize=18)
        ax1.imshow(
            deposits_in.reshape(16, 16), vmin=0, vmax=max_deposit, cmap="Purples"
        )
        ...
        ax2.imshow(
            deposits_out.reshape(16, 16), vmin=0, vmax=max_deposit, cmap="Purples"
        )
